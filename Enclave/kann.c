...
kann_t *kann_load_fp()
{
	...
	ocall_read(ann->x, sizeof(float), n_var, n_var*sizeof(float));
	ocall_read(ann->c, sizeof(float), n_const, n_const*sizeof(float));
...
int kann_train_fnn1(kann_t *ann, float lr, int mini_size, int max_epoch, int max_drop_streak, float frac_val, int n, float **_x, float **_y)
{
	...
	for (i = 0; i < max_epoch; ++i) {
		...
		if (kann_verbose >= 3) {
			...
			if (n_train_base) frintf(" (class error: %.2f%%)", 100.0f * n_train_err / n_train);
			if (n_val > 0) {
...
const float *kann_apply1(kann_t *a, float *x)
{
	...
	if (i_out < 0) return 0;
	kann_set_batch_size(a, 1);  // defines to "kad_sync_dim"
	kann_feed_bind(a, KANN_F_IN, 0, &x);
	kad_eval_at(a->n, a->v, i_out);
	//return a->v[i_out]->x;
	kad_node_t *tmp = a->v[i_out];
	float *ret = tmp->x;
