...
*/
static kad_node_t *kad_load1(kad_node_t **node)
{
	kad_node_t *p;
	p = (kad_node_t*)calloc(1, sizeof(kad_node_t));
	ocall_read(&p->ext_label, 4, 1, 4);
	ocall_read(&p->ext_flag, 4, 1, 4);
	ocall_read(&p->flag, 1, 1, 1);
	ocall_read(&p->n_child, 4, 1, 4);
	if (p->n_child) {
		int32_t j, k;
		p->child = (kad_node_t**)calloc(p->n_child, sizeof(kad_node_t*));
		ocall_read(&p->op, 2, 1, 2);
		for (j = 0; j < p->n_child; ++j) {
			ocall_read(&k, 4, 1, 4);
		...
		ocall_read(&k, 4, 1, 4);
		if (k >= 0) p->pre = node[k];
		ocall_read(&p->ptr_size, 4, 1, 4);
		if (p->ptr_size > 0) {
			p->ptr = malloc(p->ptr_size);
			ocall_read(p->ptr, p->ptr_size, 1, p->ptr_size);
		}
	} else {
		ocall_read(&p->n_d, 1, 1, 1);
		if (p->n_d) ocall_read(p->d, 4, p->n_d, p->n_d*4);
	}
	return p;
}
...
*/
kad_node_t **kad_load(int *_n_node)
{
	int32_t i, n_node;
	kad_node_t **node;
	ocall_read(&n_node, 4, 1, 4);
	node = (kad_node_t**)malloc(n_node * sizeof(kad_node_t*));
	for (i = 0; i < n_node; ++i) {
		kad_node_t *p;
		p = node[i] = kad_load1(node);
		if (p->n_child) {
			kad_op_list[p->op](p, KAD_ALLOC);
...
void kad_print_graph(int n, kad_node_t **v)
{
	int i, j;
	for (i = 0; i < n; ++i) v[i]->tmp = i;
	for (i = 0; i < n; ++i) {
		kad_node_t *p = v[i];
		printf("%d\t%x:%x\t%d\t", i, p->flag, p->ext_flag, p->ext_label);
		if (p->pre) printf("%d\t", p->pre->tmp);
		else printf(".\t");
		printf("[");
		for (j = 0; j < p->n_d; ++j) {
			if (j) printf(",");
		...
		printf("]\t");
		if (p->n_child) {
			printf("%s(", kad_op_name[p->op]);
			for (j = 0; j < p->n_child; ++j) {
				if (j) printf(",");
				printf("$%d", p->child[j]->tmp);
			}
			printf(")");
		} else printf("%s", kad_is_feed(p)? "feed" : kad_is_var(p)? "var" : kad_is_const(p)? "const" : "N/A");
		printf("\n");
	}
	for (i = 0; i < n; ++i) v[i]->tmp = 0;
}
...
void kad_check_grad(int n, kad_node_t **a, int from)
{
	...
	printf("Gradient check -- %g <=> %g @ %g -- ", s0/eps, s1/eps, f0);
	if (fabs(s1) >= rel * eps) {
		rel_err = fabsf(fabsf(s0) - fabsf(s1)) / (fabsf(s0) + fabsf(s1));
		p_m_err = fabsf(f_plus + f_minus - 2.0f * f0) / fabsf(f_plus - f_minus);
		printf("rel_err:%g p_m_err:%g -- ", rel_err, p_m_err);
		if (rel_err >= rel && rel_err > p_m_err) printf("failed\n");
		else printf("passed\n");
	} else printf("skipped\n");
	free(delta); free(g0);
}
